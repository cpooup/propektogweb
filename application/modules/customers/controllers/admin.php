...
class Admin extends Admin_Controller {
    ...
    public function index() {
        ...
        if ($this->input->get('task_log_date'))
            $filters['task_log_date'] = $this->input->get('task_log_date', TRUE);

        if ($this->input->get('customer_name'))
            $filters['customer_name'] = $this->input->get('customer_name', TRUE);

        if ($this->input->get('task_log_name'))
            $filters['task_log_name'] = $this->input->get('task_log_name', TRUE);

        if ($this->input->get('task_log_status'))
            $filters['task_log_status'] = $this->input->get('task_log_status', TRUE);

        if ($this->input->get('task_date'))
            $filters['task_date'] = $this->input->get('task_date', TRUE);
        ...
        if ($this->input->post()) {
            if ($this->input->post('clear')) {
                ...
                // apply the filter(s)
                $filter = "";
                if ($this->input->post('task_log_date'))
                    $filters['task_log_date'] = $this->input->post('task_log_date', TRUE);

                if ($this->input->post('customer_name'))
                    $filters['customer_name'] = $this->input->post('customer_name', TRUE);

                if ($this->input->post('task_log_name'))
                    $filters['task_log_name'] = $this->input->post('task_log_name', TRUE);

                if ($this->input->post('task_log_status'))
                    $filters['task_log_status'] = $this->input->post('task_log_status', TRUE);

                if ($this->input->post('task_date'))
                    $filters['task_date'] = $this->input->post('task_date', TRUE);

                if ($this->input->post('username'))
                    $filters['username'] = $this->input->post('username', TRUE);
    ...
    function export() {
        ...
        if ($users['total'] > 0) {
            ...
            foreach ($users['results'] as $key => $user) {
                ...
                $users['results'][$key][lang('export col kontering')] = lang('export kontering_monday' . $user['kontering_monday']) . lang('export kontering_tuesday' . $user['kontering_tuesday']) . lang('export kontering_wednesday' . $user['kontering_wednesday']) . lang('export kontering_thursday' . $user['kontering_thursday']) . lang('export kontering_friday' . $user['kontering_friday']);
                ...
                unset($users['results'][$key]['kontering_id']);
                unset($users['results'][$key]['kontering_monday']);
                unset($users['results'][$key]['kontering_tuesday']);
                unset($users['results'][$key]['kontering_wednesday']);
                unset($users['results'][$key]['kontering_thursday']);
                unset($users['results'][$key]['kontering_friday']);
    ...
    public function customers_task_list($id = NULL) {
        ...
        $sort = $this->input->get('sort') ? $this->input->get('sort', TRUE) : DEFAULT_SORT;
        $dir = $this->input->get('dir') ? $this->input->get('dir', TRUE) : DEFAULT_DIR;
        $date_action = $this->input->get('date') ? $this->input->get('date', TRUE) : $custom_date;
        // get filters
        $filters = array();

        if ($this->input->get('date'))
            $date_action = $this->input->get('date', TRUE);
        //$filters['task_checked_date'] = $this->input->get('date', TRUE);
        if ($this->input->get('customer_name'))
            $filters['customer_name'] = $this->input->get('customer_name', TRUE);
        if ($this->input->get('customer_email'))
            $filters['customer_email'] = $this->input->get('customer_email', TRUE);
        ...
        // save the current url to session for returning
        $this->session->set_userdata(REFERRER, THIS_URL . "?sort={$sort}&dir={$dir}&limit={$limit}&offset={$offset}{$filter}&date={$date_action}");
        ...
        if ($this->input->post()) {
            if ($this->input->post('clear')) {
                ...
                if ($this->input->post('date'))
                    $date_action = $this->input->post('date', TRUE);
                //$filters['task_checked_date'] = $this->input->post('date', TRUE);
                if ($this->input->post('customer_name'))
                    $filters['customer_name'] = $this->input->post('customer_name', TRUE);
                if ($this->input->post('customer_email'))
                    $filters['customer_email'] = $this->input->post('customer_email', TRUE);

                // redirect using new filter(s)
                redirect(THIS_URL . "?sort={$sort}&dir={$dir}&limit={$limit}&offset={$offset}{$filter}&date={$date_action}");
            }
        }
        ...
        if ($set_date_action == "") {
            foreach ($tab_date as $key => $tab) {
                if ($tab['DateAction'] != "#" && ($tab['DateFocus'] >= $time)) {
                    ...
                    $task_checked = $this->customers_model->get_checked_ByDate(mdate($DateString, $tab_date[$key]['DateFocus']), $id, $limit, $offset, $filters, $sort, $dir);
            ...
            $task_checked = $this->customers_model->get_checked_ByDate(mdate($DateString, $set_date_action), $id, $limit, $offset, $filters, $sort, $dir);
        ...
        for ($j = 1; $j <= 14; $j++) {
            $task_old = $this->customers_model->get_checked_ByDate(mdate($DateString, strtotime('-' . $j . ' day', $date_last_week)), $id, $limit, $offset, $filters, $sort, $dir);
            if (!empty($task_old)) {
    ...
    public function customers_task_checked($data, $date_today, $Month_today, $site_id) {
        ...
        foreach ($data as $item) {
            ...
            $data_kontering = Array(1 => 21, 2 => 22, 3 => 23, 4 => 24, 5 => 25);
            if ($day_now == $day_of_week) {
                if ($day_now != 6 && $day_now != 7) {
                    ...
                    $data_kontering_list = $this->customers_model->get_task_list_by_task_id($data_kontering[$day_now], $site_id);
                    if (!empty($data_entry_list)) {
                        foreach ($data_entry_list as $task) {
                            $Day = mdate($DayString, strtotime($task["start_date"]));
                            $Month = mdate($MonthString, strtotime($task["start_date"]));
                            if ($task["task_list_type"] == 0) {
                                $task_list_checked = $this->customers_model->get_task_list_checked_by_date($date_today, $task['task_list_id']);
                                if (empty($task_list_checked)) {
                                    $this->customers_model->add_task_list_checked($task['task_list_id'], $date_today);
                                }
...
//                                    }
                    ...
                    if (!empty($data_kontering_list)) {
                        foreach ($data_kontering_list as $task) {
                            $Day = mdate($DayString, strtotime($task["start_date"]));
                            $Month = mdate($MonthString, strtotime($task["start_date"]));
                            if ($task["task_list_type"] == 0) {
                                $task_list_checked = $this->customers_model->get_task_list_checked_by_date($date_today, $task['task_list_id']);
